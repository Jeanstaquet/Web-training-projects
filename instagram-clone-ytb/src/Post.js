...
function Post({postId, user, username, caption, imageUrl}) {
    ...
    useEffect(() => {
        ...
        if(postId) {
            unsubscribe = db
                ...
                //SP listenners for comments, nested listenners
                //On écoute à un endroit en particulier et pas sur toute la page
                .onSnapshot((snapshot) => {
    ...
    const postComment = (event) => {
        ...
        db.collection("posts").doc(postId).collection("comments").add({
            //utiliser l'utilisateur qui s'en enregistré
    ...
    return (
        <div className="post">
            <div className="post__header">
                <Avatar className="post__avatar" alt="Jean"></Avatar>
            <h3>{username}</h3>
            </div>
